...
document.getElementById("textareaLeft").addEventListener("input", () => {
  let inputLanguage = document
    .querySelector(".active1")
    .innerHTML.toLowerCase()
    .replace(/[{()}]/g, "");
  let outputLanguage = document
    .querySelector(".active2")
    .innerHTML.toLowerCase()
    .replace(/[{()}]/g, "");
  ...
  if (inputLanguage !== "detect language") {
    ...
    postData(url2, params2)
      .then((data) => {
        ...
        document.getElementById(
          "suggestionText"
        ).innerHTML = `<a onclick="replaceText(this)">${data[length2].input}</a> <a onclick="replaceText(this)">${data[length3].input}</a> <a onclick="replaceText(this)">${data[length].input}</a>`;
      })
      .catch((error) => {
        // console.log("something went wrong", error)
      });
    ...
    postData(url, params)
      .then((data) => {
        ...
        document.getElementById(
          "suggestionText"
        ).innerHTML = `<a onclick="changeLanguage(this)">${result} Detected</a>`;
      })
      .catch((error) => {
        console.log("something went wrong", error);
      });
